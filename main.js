...
class Minuaru extends utils.Adapter {
    ...
    async onReady() {
        ...
        if (this.config.telegram) {
            this.telegramConfig.alarm_textComes =
                this.config.telegram.alarm_textComes || '🔴 %ALARMCLASS%%NL%*%ALARMTEXT%*%NL%Room: %ALARMAREA%';
            this.telegramConfig.alarm_textGoes =
                this.config.telegram.alarm_textGoes || '🟢 %ALARMCLASS%%NL%~%ALARMTEXT%~%NL%Room: %ALARMAREA%';
            this.telegramConfig.warning_textComes =
                this.config.telegram.warning_textComes || '🟡 %ALARMCLASS%%NL%*%ALARMTEXT%*%NL%Room: %ALARMAREA%';
            this.telegramConfig.warning_textGoes =
                this.config.telegram.warning_textGoes || '🟢 %ALARMCLASS%%NL%~%ALARMTEXT%~%NL%Room: %ALARMAREA%';
            this.telegramConfig.information_textComes =
                this.config.telegram.information_textComes || '🔵 %ALARMCLASS%%NL%*%ALARMTEXT%*%NL%Room: %ALARMAREA%';
            this.telegramConfig.information_textGoes =
                this.config.telegram.information_textGoes || '🟢 %ALARMCLASS%%NL%~%ALARMTEXT%~%NL%Room: %ALARMAREA%';
        } else {
        ...
        this.checkTimeStampSchedule = schedule.scheduleJob(
            '* * * * *',
            function () {
    ...
    onUnload(callback) {
        try {
            ...
            this.log.debug(`error on unload: ${JSON.stringify(e)}`);
            callback();
        }
    }
    ...
    async handleStateChange(id, state, startUp) {
        ...
        if (state) {
            ...
            if (
                id === `${this.namespace}.stateIdToAcknowledge` &&
                state.ack === false &&
                (this.registeredStates[state.val] || this.registeredStatesCheckTimeStamp[state.val])
            ) {
            ...
            if (this.registeredStates[id] && this.registeredStates[id].enabled === true) {
                ...
                if (
                    this.registeredStates[id].type !== 'boolean' &&
                    this.registeredStates[id].type !== 'number' &&
                    this.registeredStates[id].type !== 'string'
                ) {
                ...
                if (this.registeredStates[id].type === 'boolean') {
                    ...
                    } else if (customSettings.lowEnabled === false) {
                        // state.val = true
                        alarmComes = true;
                    } else {
                        alarmGoes = true;
                    }
                } else if (this.registeredStates[id].type === 'number') {
                    // type number ?
                    ...
                    if (startUp === false && customSettings.debounce && customSettings.debounce > 0) {
                        ...
                        this.registeredStates[id].debounceTimer = setTimeout(() => {
                            ...
                            this.getForeignState(id, (err, state) => {
                                ...
                                this.log.debug(
                                    `value at alarm: ${JSON.stringify(this.registeredStates[id].valueAtAlarm)}`,
                                );
    ...
    async checkTimeStamps() {
        ...
        for (let id in this.registeredStatesCheckTimeStamp) {
            ...
            this.log.debug(
                `set Gone at first check of Timestamp necessary: ${JSON.stringify(setGoneAtFirstCheckAndTimeStampOk)}`,
            );
            if (lastTimeStamp > 0 && timeStampDiff > timeStampMaxDiff) {
                ...
                if (
                    this.registeredStatesCheckTimeStamp[id].timeStampTooOld === true ||
                    setGoneAtFirstCheckAndTimeStampOk === true
                ) {
                    // alarm goes
                    data.tsGoes = Date.now();
                    debugInfo = await databaseTools.updateAlarmGoes(this.db, data);
    ...
    async updateListData() {
        ...
        let htmlAlarmHistory = jsonToHtml.jsonToHtmlAlarms(
            alarmListData.allAlarms,
            'history',
            true,
            this.minuVisConfig,
        );
        let htmlAlarmsActive = jsonToHtml.jsonToHtmlAlarms(
            alarmListData.allActiveAlarms,
            'active',
            true,
            this.minuVisConfig,
        );
        let htmlAlarmBanner = jsonToHtml.jsonToHtmlAlarms(
            [alarmListData.allActiveNotAcknowledgedAlarms[0] || null],
            'banner',
            false,
            this.minuVisConfig,
        );
    ...
    sendToTelegram(alarmComes, data) {
        if (
            ...
            this.config.telegram.instance &&
            this.config.telegram.instance.length > 0 &&
            this.config.telegram.user &&
            this.config.telegram.user.length > 0
        ) {
            ...
            this.sendTo(
                ...
                'ask',
                {
                    ...
                    reply_markup: {
                        inline_keyboard: [[{ text: '✅', callback_data: data.stateId }]],
                    },
                },
                msg => {
