exports.initTable = async function (db) {
    let info = await db.run("CREATE TABLE IF NOT EXISTS tblMinuAru (id DOUBLE NOT NULL, tsComes DOUBLE, tsGoes DOUBLE, tsAck DOUBLE, alarmText TEXT, alarmClass TEXT, alarmArea TEXT, stateID TEXT, PRIMARY KEY (id))");
...
exports.insertAlarmComes = async function (db, data) {
    ...
    info = await db.run("INSERT INTO tblMinuAru VALUES (nextval('seq_id'), ?, 0, 0, ?, ?, ?, ?)", data.tsComes, data.alarmText, data.alarmClass, data.alarmArea, data.stateId);
    return info;
};
