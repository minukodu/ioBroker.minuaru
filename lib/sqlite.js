...
exports.insertAlarmComes = function (db, data) {
    // set all previous alalm to gone
...
exports.getAlarmListData = function (db) {
    ...
    // get number of active, but not acknoledged alarms
    stmt = db.prepare("SELECT COUNT (tsComes) AS nbAlarms FROM tblMinuAru WHERE tsGoes = 0 AND tsAck = 0");
    alarmListdata.nbActiveAlarmsNotAcknowledged = stmt.get().nbAlarms;
