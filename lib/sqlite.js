exports.initDatabase = function (dbName) {
...
exports.initTable = function (db) {
    let stmt = db.prepare(
        'CREATE TABLE IF NOT EXISTS tblMinuAru (id INTEGER NOT NULL PRIMARY KEY AUTOINCREMENT, tsComes INT, tsGoes INT, tsAck INT, alarmText TEXT, alarmClass TEXT, alarmArea TEXT, stateID TEXT)',
    );
    let info = stmt.run();
    return info;
};
...
exports.insertAlarmComes = function (db, data) {
    ...
    stmt = db.prepare(
        'INSERT INTO tblMinuAru VALUES (NULL, @tsComes, 0, 0, @alarmText, @alarmClass, @alarmArea, @stateId)',
    );
    info = stmt.run(data);
    return info;
};
